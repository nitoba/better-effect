...
PostgreSQL pool, schema, and namespace for providers that share a queue. Outbox transaction helpers
participate in a transaction that the application already owns; they never turn a remote publish
