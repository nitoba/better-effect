...
MongoDB may retry a transaction callback, so domain writes must be safe to retry. After commit,
compose the Runtime-owned publisher beside the existing Worker; it routes to the `JobStore` Service
token and the Worker delivers the Job:
...
// Add PublisherLive and the existing EmailWorkerLive to the same Runtime as MongoOutboxStore.layer(...).
...
deterministic Job ID as the downstream idempotency key. If the first argument is a `MongoClient`
instead of `db`, pass the database as `options.db`. See the [outbox transaction guide](../../outbox)
