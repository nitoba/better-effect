...
Native cross-realm Responses and compatible Web Response implementations are
accepted. Malformed values, including forged `Response.prototype` objects,
are rejected with `TypeError` before they are returned.
