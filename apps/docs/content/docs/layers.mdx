...
contract. `LayerProvided`, `LayerRawRequired`, `LayerMissing` and `LayerSpecs`
remain compatible top-level exports. The source-only `AnyLayer` and
`CompleteLayer` names are not package-root exports; use `Layer.Any` and
`Layer.Complete` for those associated helpers.
