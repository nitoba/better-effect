---
title: Troubleshooting
description: Diagnose the most common type and lifecycle failures.
icon: CircleQuestionMark
---

<Accordions>
  <Accordion title="TypeScript says a Service is missing" id="missing-service">
    Read the named diagnostic, for example `MissingDependencies<Logger | Cache>`. The generic
    argument is the exact tagged Service instance union that is unavailable. Add those providers to
    the Layer, merge the Layers, or use an explicit compatible override for a test.
  </Accordion>
  <Accordion title="I used Layer.merge and got a duplicate error" id="duplicate-layer">
    `merge` never silently replaces a provider. Use `Layer.override(base, replacement)` when
    replacement is intentional, and ensure same-tag contracts are compatible.
  </Accordion>
  <Accordion title="A Service resolves outside Runtime" id="service-outside-runtime">
    `yield* Service` and `ServiceRuntime.resolve` need an active resolver. Run the program through
    `Runtime.run`, or wrap integration code with `ServiceRuntime.run(resolver, callback)`.
  </Accordion>
  <Accordion title="Scope says it is not configured" id="scope-outside-runtime">
    `Scope.current`, `yield* Scope`, `Effect.add` and `Effect.acquireRelease` require an active
    Scope. Use a Runtime execution, `Scope.run`, or `Scope.provide` with an existing Scope.
  </Accordion>
  <Accordion title="My resource did not release" id="resource-not-released">
    Confirm acquisition succeeded, the resource was registered in the current Scope, and the release
    callback or disposal protocol is callable. For `Effect.add`, the object must expose
    `Symbol.asyncDispose` or `Symbol.dispose`.
  </Accordion>
  <Accordion title="Cleanup hid my original error" id="cleanup-primary-error">
    Cleanup failures are secondary when the program failed. Configure `onCleanupFailure` to observe
    them. A successful program can surface a cleanup failure because there is no more specific
    primary failure.
  </Accordion>
  <Accordion title="Explicit context storage rejects overlap" id="explicit-context-overlap">
    `ExplicitRuntimeContextStorage` is for hosts that cannot propagate async context. One instance
    supports one non-overlapping async flow; use the default Node/Bun storage for concurrent work.
    Separate explicit storage instances do not provide concurrent isolation on hosts without async
    context propagation.
  </Accordion>
  <Accordion title="The default Hono error is generic" id="hono-error-redaction">
    Default Hono failure responses redact `Error.message`. Configure `onFailure` only when you need
    a deliberate, safe domain representation; do not serialize arbitrary exceptions to clients.
  </Accordion>
  <Accordion title="Layer.gen yielded an unsupported value" id="bad-layer-yield">
    Layer generators may yield Service constructors only; their public requirement metadata uses
    the corresponding instance types. Use `Result.await` and `Effect.gen` for Result operations in
    application programs; use `Layer.scopedGen` for contextual acquisition and cleanup.
  </Accordion>
</Accordions>

## A debugging checklist

1. Confirm the Service tag is non-empty and stable.
2. Inspect `Layer.Provided`, `Layer.Required` and `Layer.Complete` when a
   composition root fails to typecheck.
3. Verify the program runs inside the intended Runtime.
4. Check whether the resource belongs to the root Scope or execution Scope.
5. Preserve the primary Result/exception and inspect cleanup diagnostics
   separately.
