...
const response =
  yield *
  auth.api.getSession.asResponse({
    headers: request.headers
  })
const withHeaders =
  yield *
...
`code`, and `statusCode`, or translate it to a domain failure at your own
boundary. Do not log `body`, `headers`, or `cause` indiscriminately: those
fields can contain cookies, tokens, request data, or adapter details even
though the error's serialized form redacts them.
...
The package is server-side and framework-neutral. It does not create Better
Auth instances, choose database adapters, run migrations, own environment or
configuration, own Runtime shutdown, provide client hooks, add retries,
implement a roles or authorization policy engine, or map Better Auth errors
automatically to application-domain failures. Keep those responsibilities in
the application and use the package only at the server boundary where Better
Auth operations enter a typed Program.
