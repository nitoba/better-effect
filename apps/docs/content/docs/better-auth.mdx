...
The prepared v0.1 peer matrix applies to the framework-neutral `.` entry point:
...
The optional `/hooks` and `/hono` entry points are planned v0.2 integrations.
Hono is an optional peer of `/hono` at `>=4.0.0`; it is not needed by the main
or `/hooks` entry points.
...
## Planned v0.2: Hooks and plugin middleware
