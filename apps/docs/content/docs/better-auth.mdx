...
# Optional request-scoped Hono integration:
bun add hono
```
...
- `better-result` `^3.0.0`
- Hono `>=4.0.0` for the optional `/hono` entry point
- TypeScript `>=5.7.0`
...
Install Hono when using the optional `better-effect-better-auth/hono` entry
point. `BetterAuthHono.session` creates a typed current-session Service linked
to one exact Better Auth Service:
...
// Leave Better Auth's handler on its normal route.
...
only `null` to `Unauthenticated`. Better Auth API failures and unexpected
failures retain their types, and `HonoEffect`'s `onFailure` callback decides the
HTTP response.
...
Program. The optional `better-effect-better-auth/hono` entry point adds only the
request-scoped session helper; it does not replace or wrap Better Auth's Web
