...
const runtime = await Runtime.make(Layer.merge(coreValues, Auth.layer))
```

`coreValues` is a value Layer owned by the application, and `Auth.layer` is
...
Stop accepting requests before shutdown, then dispose the application Runtime
and close caller-owned shared resources last:
