...
test("explicit registry ids belong to the concrete class, not its generated base", () => {
  class Registered extends Z.Class("RegisteredWithId", {
    id: "urn:better-effect-schema:registered",
    title: "Registered"
  })({ value: z.string() }) {}
  ...
  assert.equal(Registered.meta().id, "urn:better-effect-schema:registered")

  const codec = Registered.codec
  assert.equal(z.globalRegistry.get(Registered).id, "urn:better-effect-schema:registered")
