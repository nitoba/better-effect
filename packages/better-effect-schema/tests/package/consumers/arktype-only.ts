...
import { Schema, ArkTypeAdapter } from 'better-effect-schema/arktype'

const Local = CoreSchema.with(ArkTypeAdapter)
const User = type({ id: 'string', name: 'string' })
const Length = type('string').pipe((value) => value.length, type('number'))

const decoded = Schema.decode(Length, 'arktype-morph')
