...
**Schema classes and typed validation boundaries for the better-effect ecosystem.**

> **Release blocked:** this package is in a transitional workspace-only state. The
> implementation and root entrypoint remain Zod-backed until the provider adapters
> are integrated. Do not publish `better-effect-schema` from this revision.

`better-effect-schema` currently turns Zod object schemas and bidirectional codecs into real TypeScript classes. The same declaration can validate encoded input, construct domain objects, preserve class behavior, and encode instances back to a boundary representation.
...
```bash
npm install better-effect-schema zod better-result better-effect
```

```bash
bun add better-effect-schema zod better-result better-effect
```
...
`better-effect-schema` intentionally does not:
