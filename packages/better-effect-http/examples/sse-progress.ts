    ...
    try {
      const result = await runtime.run(
        Effect.fn(async function* () {
          ...
          const events = http.sse('/jobs/job-1/events', {
            events: { progress, completed },
            reconnect: false
          })
          const percentages: number[] = []
          const terminal = yield* events.takeUntil(
            (message) => {
          ...
          if (terminal.value.event !== 'completed')
            return Result.err(new Error('wrong SSE terminal'))
          return Result.ok({ percentages, terminal: terminal.value.data.jobId })
        })
      )

      if (Result.isError(result)) throw result.error
      if (result.value.terminal !== 'job-1') throw new Error('SSE did not reach completed')
