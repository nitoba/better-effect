...
local function validEvent(event)
  ...
  local types = {
    ["job-enqueued"] = true, ["job-claimed"] = true, ["job-completed"] = true,
  ...
  local allowed = {
    type=true, recordedAtMs=true, jobId=true, queue=true, name=true, version=true,
    state=true, attempt=true, delivery=true, workerId=true, outcome=true,
    failureKind=true, duplicate=true, attributes=true
  }
...
local function appendEvent(item)
  ...
  local id = redis.call("XADD", keys.events, "*", "data", cjson.encode(item.event))
  ...
  if retention.count then
    removed = removed + redis.call("XTRIM", keys.events, "MAXLEN", "=", tostring(retention.count))
  end
  if removed > 0 then
