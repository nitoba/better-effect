...
local function okReply(status, id, version)
  local result = {"ok", replyName, status}
  if id ~= nil then result[#result + 1] = id end
  if version ~= nil then result[#result + 1] = tostring(version) end
  return result
end
...
local function validEvent(event)
  ...
  local types = { ["job-enqueued"]=true, ["job-claimed"]=true, ["job-completed"]=true, ["job-retry-scheduled"]=true, ["job-failed"]=true, ["job-cancelled"]=true, ["job-cancel-requested"]=true, ["job-released"]=true, ["job-stalled-recovered"]=true, ["job-promoted"]=true, ["job-admin-retried"]=true, ["job-removed"]=true, ["queue-paused"]=true, ["queue-resumed"]=true, ["flow-fan-out"]=true, ["flow-child-results-recorded"]=true, ["flow-cancelled"]=true, ["flow-cascaded"]=true, ["flow-outbox-appended"]=true, ["schedule-upserted"]=true, ["schedule-removed"]=true, ["schedule-ticked"]=true, ["schedule-paused"]=true, ["schedule-resumed"]=true, ["controls-reconciled"]=true, ["controls-claimed"]=true, ["controls-settled"]=true, ["controls-released"]=true, ["controls-stalled-recovered"]=true, ["controls-cancelled"]=true }
...
local function appendEvent(item)
  ...
  for _, event in ipairs(events) do
    ...
    if retention.ageMs then
      ...
      if cutoff > 0 then
        local previous = redis.call("XREVRANGE", item.keys.events, "(" .. tostring(cutoff) .. "-0", "-", "COUNT", "1")
        if previous[1] and previous[1][1] then trimmedThrough = previous[1][1] end
      end
      redis.call("XTRIM", item.keys.events, "MINID", "=", tostring(cutoff) .. "-0")
    end
    if retention.count then
      local previous = redis.call("XREVRANGE", item.keys.events, "+", "-", "COUNT", retention.count + 1)
      local boundary = previous[retention.count + 1]
      if boundary and boundary[1] then trimmedThrough = boundary[1] end
      redis.call("XTRIM", item.keys.events, "MAXLEN", "=", tostring(retention.count))
    end
    if trimmedThrough then redis.call("HSET", item.keys.eventsMeta, "trimmedThrough", trimmedThrough) end
  end
  return true
end
...
if p.mode == "cleanup-active" then
  if type(p.active) ~= "string" or type(p.items) ~= "table" or #p.items > 128 or
    not keyTypeIs(p.active, "zset") then
    return errorReply("MQ_INVALID_ARGUMENT")
  end
  for _, item in ipairs(p.items) do
    if type(item) ~= "table" or type(item.id) ~= "string" or item.id == "" or
      type(item.job) ~= "string" or item.job == "" or
      type(item.score) ~= "string" and type(item.score) ~= "number" or
      not safeNumber(item.score) or not keyTypeIs(item.job, "hash") then
      return errorReply("MQ_INVALID_ARGUMENT")
    end
  end
  local cleaned = 0
  for _, item in ipairs(p.items) do
    local expectedScore = safeNumber(item.score)
    local currentScore = redis.call("ZSCORE", p.active, item.id)
    if expectedScore and currentScore then
      local actualScore = safeNumber(currentScore)
      if actualScore == expectedScore then
        ...
        elseif state ~= "active" then
          remove = item.state == nil or item.state == state
        else
          remove = item.state == "active" and type(item.token) == "string" and
            item.token ~= "" and redis.call("HGET", job, "leaseToken") == item.token and
            redis.call("HGET", job, "leaseExpiresAt") == false
        end
        if remove then
          redis.call("ZREM", p.active, item.id)
          cleaned = cleaned + 1
        end
      end
    end
  end
  return {"ok", "remove", "cleaned", tostring(cleaned)}
end
if p.mode == "repair-active" then
  if type(p.active) ~= "string" or type(p.items) ~= "table" or #p.items > 128 or
    not keyTypeIs(p.active, "zset") then
    return errorReply("MQ_INVALID_ARGUMENT")
  end
  for _, item in ipairs(p.items) do
    if type(item) ~= "table" or type(item.id) ~= "string" or item.id == "" or
      type(item.job) ~= "string" or item.job == "" or type(item.score) ~= "number" or
      type(item.leaseExpiresAt) ~= "number" or type(item.token) ~= "string" or item.token == "" or
      not safeNumber(item.score) or not safeNumber(item.leaseExpiresAt) or not keyTypeIs(item.job, "hash") then
      return errorReply("MQ_INVALID_ARGUMENT")
    end
  end
  local repaired = 0
  for _, item in ipairs(p.items) do
    local expectedScore, desiredScore = safeNumber(item.score), safeNumber(item.leaseExpiresAt)
    local currentScore = redis.call("ZSCORE", p.active, item.id)
    if expectedScore and desiredScore and currentScore and safeNumber(currentScore) == expectedScore and
      keyTypeIs(item.job, "hash") and redis.call("HGET", item.job, "state") == "active" and
      redis.call("HGET", item.job, "leaseToken") == item.token and
      safeNumber(redis.call("HGET", item.job, "leaseExpiresAt") or "") == desiredScore then
      redis.call("ZADD", p.active, desiredScore, item.id)
      repaired = repaired + 1
    end
  end
  return {"ok", "remove", "repaired", tostring(repaired)}
end
if not p.mode or p.mode ~= "remove" then return errorReply("MQ_INVALID_ARGUMENT") end
...
if p.mode == "write" then
  ...
  if result == "already" then
    ...
    return okReply("already", redis.call("HGETALL", p.keys.job), latest[1] or "")
  end
  if result == "unsafe" then return errorReply("MQ_UNSAFE_INTEGER") end
  if result == "corrupt" then return errorReply("MQ_CORRUPT_COUNTER") end
  if result == "invalid" then return errorReply("MQ_INVALID_ARGUMENT") end
