...
import { Codec, JobScheduleStore, JobStore, Queue } from 'better-effect-mq'
...
const payloads: readonly JsonValue[] = [
  'null', 'true', 'false', '123', '0', '{}', '[]', '"quoted"',
  '{"value":null}', '', 'ação 💾', null, true, false, 123,
  ['null', { value: '123' }], { value: 'null', nested: ['true', null] }
]
  ...
  test(`SQLite schedules preserve decoded JSON ${JSON.stringify(payload)} through admin and ticks`, async () => {
    ...
    try {
      ...
      const job = Queue.define('schedule-json').job('echo', {
        version: 1, payload: Codec.json<JsonValue>()
      })
      const record: ScheduleRecord = {
        key: 'payload', group: 'json', queue: job.identity.queue, job: job.identity,
        cron: undefined, everyMs: 1000, timeZone: 'UTC', payload, metadata: {},
        priority: 0, attemptsMax: 1, backoff: undefined, timeoutMs: undefined,
        misfire: { strategy: 'run-once' }, overlap: 'allow', paused: false,
        revision: 0, nextRunAtMs: 1000, lastScheduledAtMs: undefined,
        lastJobId: undefined, createdAtMs: 0, updatedAtMs: 0
      }
      ...
      expect((await store.listSchedules({ group: 'json' })).unwrap().map((item) => item.payload)).toEqual([payload])
      expect((await store.dueSchedules({ nowMs: 1000 })).unwrap().map((item) => item.payload)).toEqual([payload])
      ...
      assert.ok(inspected)
      const changed = (await store.upsertSchedule({ ...inspected, priority: 2, updatedAtMs: 50 })).unwrap()
      expect(changed.changed).toBe(true)
      expect(changed.record.payload).toEqual(payload)
      expect(stored()).toEqual(expected)
      const command = {
        key: selector, expectedRevision: changed.record.revision,
        expectedRunAtMs: 1000, nowMs: 1000,
        decision: { occurrences: [1000], nextRunAtMs: 2000 }
      }
      const stale = (await store.tickSchedule({ ...command, expectedRevision: command.expectedRevision - 1 })).unwrap()
      ...
      try { await runtime.dispose() } finally { database.close() }
    }
  })
}
