...
try {
  ...
  try {
    ...
    for (const [index, payload] of payloads.entries()) {
      ...
      assert.deepEqual(
        fired.jobs.map((job) => job.payload),
        [payload]
      )
    }
  } finally {
  ...
  try {
    ...
    for (const [index, payload] of payloads.entries()) {
      ...
      const saved = reopened
        .prepare('SELECT payload FROM better_effect_mq_jobs WHERE id=?')
        .get(record.lastJobId)
      assert.equal(saved.payload, JSON.stringify(payload))
    }
    console.log(
      `PASS packed ${host}: schedule JSON survives pause/resume, native ticks and file reopen (${payloads.length} values)`
    )
  } finally {
    reopened.close()
  }
