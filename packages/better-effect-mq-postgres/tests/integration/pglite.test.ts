...
describe('PostgreSQL foundation via PGlite', () => {
  ...
  test('migrates, validates, and remains idempotent on a fresh custom schema', async () => {
    ...
    try {
      ...
      await expect(client.validate()).resolves.toMatchObject({ schema, version: 5 })
    } finally {
      await database.close()
    }
  })
  ...
  test('upgrades a pre-versioned schema from the previous baseline to the current migration', async () => {
    ...
    try {
      ...
      expect(migrated).toMatchObject({ applied: [1, 2, 3, 4, 5], schema, version: 5 })
      ...
      expect(history.rows).toEqual([
        {
          version: 5,
          applied_at_ms: 2,
          checksum: migrationManifestChecksum(migrations, 5)
        }
      ])
      await expect(client.validate()).resolves.toMatchObject({ schema, version: 5 })
    } finally {
