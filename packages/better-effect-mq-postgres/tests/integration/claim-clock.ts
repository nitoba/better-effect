...
try {
  ...
  const earlier = valueOf(await jobs.enqueue({
    job: identity, payload: { key: 'earlier' }, now: 100, runAt: 100, attemptsMax: 1
  }))
  ...
  const later = valueOf(await jobs.enqueue({
    job: identity, payload: { key: 'later' }, now: 200, runAt: 100, attemptsMax: 1
  }))
  const request = {
    queue, accepted: [identity], workerId: valueOf(makeWorkerId('claim-clock')),
    limit: 2, leaseDurationMs: 2_000, now: 150
  }
  ...
  assert.deepEqual(first.value.jobs.map((job) => job.id), [earlier.job.id])
  ...
  assert.deepEqual(second.jobs.map((job) => job.id), [later.job.id])
  assert.equal(second.jobs[0]?.leaseExpiresAt, 2_200)
  console.log('PASS: stale claim samples defer newer records without dropping older work or backdating leases')
} finally {
  try {
