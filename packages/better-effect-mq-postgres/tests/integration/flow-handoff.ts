...
async function verifyLeases(pool: Pool, schema: string): Promise<void> {
  ...
  try {
    ...
    for (let index = 0; index < 2; index += 1) {
      valueOf(await jobs.enqueue({ job: identity, payload: { index }, runAt: now, now, attemptsMax: 2 }))
    }
    const claimed = valueOf(await jobs.claim({
      queue, accepted: [identity], workerId: valueOf(makeWorkerId('lease-regression')),
      limit: 2, leaseDurationMs: 60_000, now: now + 1
    }))
    const [parent, ordinary] = claimed.jobs
    assert.ok(parent && ordinary)
    const childId = valueOf(makeFlowChildId({ parentStoreKey: JobStore.serviceTag, flowId: parent.id, childKey: 'one' }))
    const request = valueOf(makePreparedEnqueue({
      protocolVersion: 1, identity: { queue: 'lease-regression', name: 'child', version: 1 },
      id: childId, payload: { ok: true }, metadata: {}, priority: 0, runAt: now + 2, now: now + 2, attemptsMax: 1
    }))
    valueOf(await flows.fanOut({
      flowId: parent.id, flowName: 'lease-regression', parentStoreKey: JobStore.serviceTag,
      leaseToken: parent.leaseToken, depth: 1, failFast: false, now: now + 2,
      children: [{ childKey: 'one', name: 'child', version: 1, storeKey: JobStore.serviceTag, childJobId: childId, request }]
    }))
    const heartbeat = valueOf(await jobs.heartbeat({
      leases: [parent, ordinary].map((job) => ({ jobId: job.id, leaseToken: job.leaseToken })),
      leaseDurationMs: 60_000, now: now + 3
    }))
    ...
    const released = await jobs.release({ jobId: parent.id, leaseToken: parent.leaseToken, now: now + 4 })
    assert.ok(Result.isError(released) && LeaseLostError.is(released.error))
    const settled = await jobs.settle({ jobId: parent.id, leaseToken: parent.leaseToken, outcome: { type: 'complete' }, now: now + 4 })
    ...
    console.log('PASS: mixed heartbeat preserves ordinary lease; stale release/settle cannot mutate suspended parent')
  } finally {
    await flows.dispose()
  }
}
...
async function verifyWorker(pool: Pool, schema: string): Promise<void> {
  ...
  const parent = queue.job('parent', {
    version: 1, payload: Codec.json<{ readonly empty: boolean }>(),
    result: Codec.json<{ readonly completed: number; readonly failed: number }>()
  })
  const child = queue.job('child', {
    version: 1, payload: Codec.json<{ readonly fail: boolean }>(),
    result: Codec.json<{ readonly ok: boolean }>(), failure: Codec.json<{ readonly code: string }>()
  })
  const definition = Flow.define('handoff-regression', { parent, children: [child] as const, onChildFailure: 'continue' })
  const phases: Array<{ readonly id: string; readonly phase: string; readonly delivery: number; readonly lease: string }> = []
  const workerErrors: unknown[] = []
  const handler = Flow.handle(definition, {
    fanOut: (payload) => Effect.fn(async function* () {
      const context = yield* JobContext
      const state = await pool.query<{ state: string; lease_token: string; delivery_count: string }>(
        `SELECT state, lease_token, delivery_count FROM "${schema}".better_effect_mq_jobs WHERE id=$1`, [context.jobId]
      )
      const record = state.rows[0]
      assert.ok(record && record.state === 'active' && record.lease_token)
      phases.push({ id: context.jobId, phase: 'fanOut', delivery: Number(record.delivery_count), lease: record.lease_token })
      return Result.ok([Flow.children(child, payload.empty ? [] : [
        { key: 'bad', payload: { fail: true } }, { key: 'good', payload: { fail: false } }
      ])] as const)
    }),
    collect: (_payload, results) => Effect.fn(async function* () {
      const context = yield* JobContext
      const state = await pool.query<{ state: string; lease_token: string; delivery_count: string }>(
        `SELECT state, lease_token, delivery_count FROM "${schema}".better_effect_mq_jobs WHERE id=$1`, [context.jobId]
      )
      const record = state.rows[0]
      assert.ok(record && record.state === 'active' && record.lease_token)
      phases.push({ id: context.jobId, phase: 'collect', delivery: Number(record.delivery_count), lease: record.lease_token })
  ...
  const live = Layer.complete(Layer.merge(
    PostgresJobStore.layer({ pool, schema }), Layer.succeed(FlowStore, FlowStore.of(flows)), ClockLive,
    token.layer(() => ({
      handlers: [Worker.handle(child, (payload) => Effect.fn(async function* () {
        yield* JobContext
        return payload.fail ? Result.err({ code: 'child-failed' }) : Result.ok({ ok: true })
      }))] as const,
      flows: [handler] as const, concurrency: 1, pollIntervalMs: 5, flowSweepIntervalMs: 20,
      leaseDurationMs: 2_000, heartbeatIntervalMs: 100,
  ...
  try {
    ...
    for (const empty of [false, true]) {
      ...
      let completed = false
      while (Date.now() < end) {
        const rows = await pool.query<{ state: string; result: { completed: number; failed: number } }>(
