...
const handler = Flow.handle(definition, {
  fanOut: (payload) => Effect.fn(async function* () {
    ...
    return Result.ok([Flow.children(child, [{ key: context.jobId, payload: business.prefix + payload }])] as const)
  }),
...
export type ContextIsProvidedPerAttempt = Assert<Extract<Requirements, JobContext> extends never ? true : false>
export type BusinessDependencyMustStillBeProvided = Assert<BusinessService extends Requirements ? true : false>
