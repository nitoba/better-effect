...
async function unwrap<Value, Failure>(operation: Result<Value, Failure> | PromiseLike<Result<Value, Failure>>): Promise<Value> {
...
test('a terminal child reports through the FlowStore token rather than the JobStore token', async () => {
  ...
  const parent = queue.job('parent', { version: 1, payload: Codec.string, result: Codec.string, store: storeToken })
  const child = queue.job('child', { version: 1, payload: Codec.string, result: Codec.string, store: storeToken })
  const definition = Flow.define('report-route-regression', { parent, children: [child] as const, onChildFailure: 'continue' })
  const flowId = JobId.make('parent-report-route').unwrap()
  const childJobId = makeFlowChildId({ parentStoreKey: storeToken.serviceTag, flowId, childKey: 'one' }).unwrap()
  const request = makePreparedEnqueue({
    protocolVersion: 1, identity: child.identity, id: childJobId, payload: 'result',
    metadata: {
    ...
    priority: 0, runAt: 0, now: 0, attemptsMax: 1
  }).unwrap()
  await unwrap(flows.fanOut({
    flowId, flowName: definition.name, parentStoreKey: storeToken.serviceTag,
    depth: 1, leaseToken: LeaseToken.make('independent-manifest').unwrap(), failFast: false, now: 0,
    children: [{ childKey: 'one', name: child.name, version: child.version, storeKey: storeToken.serviceTag, childJobId, request }]
  }))
  ...
  const live = Layer.complete(Layer.merge(
    ...
    token.layer(() => ({
      handlers: [Worker.handle(child, (value) => Effect.fn(async function* () {
