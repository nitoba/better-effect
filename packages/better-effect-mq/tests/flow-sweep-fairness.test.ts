...
test('bounded flow sweeps eventually visit every known parent without relying on report delivery', async () => {
  ...
  const definition = Flow.define('flow-sweep-fairness', { parent, children: [child] as const, onChildFailure: 'continue' })
  const ids = ['first', 'second', 'third', 'fourth'].map((id) => JobId.make(`sweep-${id}`).unwrap())
  for (const flowId of ids) {
    const childJobId = makeFlowChildId({ parentStoreKey: JobStore.serviceTag, flowId, childKey: 'one' }).unwrap()
    await unwrap(flows.fanOut({
      ...
      now: 0,
      children: [{
        childKey: 'one', name: child.name, version: child.version, storeKey: JobStore.serviceTag, childJobId,
        request: makePreparedEnqueue({
          protocolVersion: 1, identity: child.identity, id: childJobId, payload: flowId,
          metadata: {}, priority: 0, runAt: 0, now: 0, attemptsMax: 1
        }).unwrap()
      }]
    }))
  }
