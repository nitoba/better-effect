...
the Service identity while retaining an explicit self type for exact
TypeScript inference. A widened `string` value MUST be rejected at compile
time, and the legacy untagged self-type declaration form MUST no longer be the
...
- **THEN** `EffectRequirements` MUST contain
  `ServiceToken<'Database', Database>` and MUST preserve the literal tag and
  self type
